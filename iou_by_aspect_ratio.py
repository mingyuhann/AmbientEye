"""
IoU vs. Pupil Aspect Ratio — Three-panel figure
================================================
...
Output: <OUTPUT_DIR>/iou_by_aspect_ratio.{pdf,png}
"""
...
def plot_figure(oe, te, ou_bal, ou_lat, ou_med, ou_face, ou_away, output_dir):
    ...
    def draw(ax, datasets, title):
        for label, df, color, marker, ls in datasets:
            ctrs, means, stds, _ = bin_iou(df, "aspect_ratio", BINS)
        ...
        ax.set_xlabel("Pupil Aspect Ratio  (minor / major)", fontsize=FS_LABEL)
        ax.set_ylabel("Mean IoU", fontsize=FS_LABEL)
        ax.tick_params(labelsize=FS_TICK)
        ax.set_xlim(0.25, 1.05); ax.set_ylim(0, 1.05)
    ...
    for suffix, dpi in [(".pdf", 300), (".png", 200)]:
        out = output_dir / f"iou_by_aspect_ratio{suffix}"
...
def load_and_enrich():
    ...
    # Panel B: eye1=lateral, eye0=medial
    ...
    # Panel C: sunfacing vs sunoccluded
